    ...
    def cost_bar_search(self):
        ...
        for _time in time_list:
            for idx, _type in enumerate(type_list):
                ...
                sql = "select count(1) from Charge_history where charge_date>='{}' " \
                      "and charge_date<='{}' and bike_type='{}'".format(start, end, _type)
                c.execute(sql)
                cost = int(c.fetchone()[0]) * 10
                sql = "select error_type from Repair_history inner join Bike_info on " \
                      "Bike_info.bike_id=Repair_history.bike_id where repair_date>=? and repair_date<=? and type=?"
                c.execute(sql, (start, end, _type))
                records = c.fetchall()
                for error_type in records:
                    if error_type[0] == "Wheels":
...
class Location:
    ...
    def plot_graph(self, S_W, S_H):
        ...
        # electric_bikes and electric_scooters
...
class Maintenance:
    ...
    def show_percent_repaired(self, S_W=None, S_H=None):
        ...
        soft_colors = ['#e3716e', '#eca680', '#7ac7e2', '#f7df87', '#54beaa', '#af8fd0']
        ...
        for i, bike_type in enumerate(bike_types):
            ...
            axs[i].pie(counts, labels=messages, autopct='%1.1f%%', startangle=140,colors=soft_colors)
