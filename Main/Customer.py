...
class RentBike:
    ...
    def show_frame_make_payment(self):
        ...
        return_riding = self.get_current_trackingInfo()
        self.l_bike_id_show_2["text"] = return_riding[1]
        self.l_bike_type_show_2["text"] = return_riding[7]
        self.l_start_time_show_2["text"] = return_riding[4]
        self.l_end_time_show_2["text"] = return_riding[5]
        self.l_riding_time_show_2["text"] = return_riding[6]
        self.l_price_show_2["text"] = return_riding[-4]
    ...
    def confirm_unlock(self):
        click = messagebox.showinfo("Unlock", "Unlocking bike is flashing!")
        if click:
            insert_tracking_history = CusOpenDatabase()
            current_bikeInfo = insert_tracking_history.insert_rentTrackData(self.comb_type.get(),
                                                                            self.comb_location.get())
            self.l_bike_id_show["text"] = current_bikeInfo[1]
            self.l_bike_type_show["text"] = current_bikeInfo[7]
            self.start_stop()
            self.ShowSuccess()
    ...
    def ShowSuccess(self):
        ...
        messagebox.showinfo("Unlocking password", "{}".format(unlockpassword_2))
    ...
    def paied_successfully(self):
        self.charge_fees()
        messagebox.showinfo("finished payment", "Thanks for your payment to finish renting!")
        self.show_frame_rent()
    ...
    def frent(self):
        ...
        self.comb_type = ttk.Combobox(self.frame_rent, values=["electric_bike", "electric_scooter"], font=FONT)
        ...
        self.l_location.place(x=S_W * 300 / 1440, y=S_H * 200 / 900)
        self.l_type.place(x=S_W * 300 / 1440, y=S_H * 350 / 900)
        self.l_avail.place(x=S_W * 600 / 1440, y=S_H * 500 / 900)
        # place combs:
        self.comb_location.place(x=S_W * 600 / 1440, y=S_H * 200 / 900, width=S_W * (500 / 1440),
                                 height=S_H * (40 / 900))
        self.comb_type.place(x=S_W * 600 / 1440, y=S_H * 350 / 900, width=S_W * (500 / 1440), height=S_H * (40 / 900))
        # place buttons:
        self.btn.place(x=S_W * 600 / 1440, y=S_H * 650 / 900, width=200, height=50)
        self.btn_avail.place(x=S_W * 300 / 1440, y=S_H * 500 / 900, height=50)

    def SOS(self):
        adr = CusOpenDatabase()
        current_rent_bike = self.get_current_trackingInfo()
        address = adr.get_address(current_rent_bike[2])[0][0]
        messagebox.showinfo("SOS", "Your location {} have been reported to the Police Officer ! ! ! "
                                   "The rescuers will be here soon.".format(address))
    ...
    def fcurrent_riding(self):
        ...
        # create labels
        self.frame_current_riding_title = Label(self.frame_current_riding, text='Current Riding', font=FONT_TOP)
        ...
        self.l_bike_id_show = Label(self.frame_current_riding, text="{}".format(current_rent_bike[1]),
                                    font=FONT_TOP)
        self.l_bike_type_show = Label(self.frame_current_riding, text='{}'.format(current_rent_bike[7]),
                                      font=FONT_TOP)
        ...
        adr = CusOpenDatabase()
        address = adr.get_address(current_rent_bike[2])
        values = [float(x) for x in address[0][0].strip('[]').split(', ')]
        self.l_map = LabelFrame(self.frame_current_riding)
        self.l_map.place(x=S_H * 1000 / 1440, y=S_H * (2 / 10), width=S_W * (800 / 1440), height=S_H * (600 / 900))
        ...
        map_widget.set_marker(values[0], values[1], text="{}".format(current_rent_bike[2]))
        map_widget.grid()
        ...
        # place label
        self.frame_current_riding_title.place(x=S_W * 600 / 1440, y=S_H * 100 / 900)
        self.l_bike_id.place(x=S_W * 50 / 1440, y=S_H * 250 / 900)
        self.l_bike_id_show.place(x=S_W * 300 / 1440, y=S_H * 250 / 900)
        self.l_bike_type.place(x=S_W * 50 / 1440, y=S_H * 350 / 900)
        self.l_bike_type_show.place(x=S_W * 300 / 1440, y=S_H * 350 / 900)
        self.l_start_time.place(x=S_W * 50 / 1440, y=S_H * 450 / 900)
        self.l_startTime_show.place(x=S_W * 300 / 1440, y=S_H * 450 / 900)
        self.l_riding_time.place(x=S_W * 50 / 1440, y=S_H * 550 / 900)
        self.l_ridingTime_show.place(x=S_W * 300 / 1440, y=S_H * 550 / 900)
        self.l_select_return_location.place(x=S_W * 50 / 1440, y=S_H * 650 / 900)

        self.btn_SOS = Button(self.frame_current_riding, text='SOS', font=FONT,
                              command=self.SOS)
        self.btn_SOS.place(x=S_W * 1300 / 1440, y=S_H * 100 / 900)
        self.btn_SOS["bg"] = "red"

        # place button:
        self.btn_return_bike.place(x=S_W * 200 / 1440, y=S_H * 750 / 900, width=S_W * (200 / 1440),
                                   height=S_H * (40 / 900))

        # place combobox:
        self.comb_location_return.place(x=S_W * 300 / 1440, y=S_H * 650 / 900, width=WIDTH,
                                        height=HEIGHT)

    def fmake_payment(self):

        # create labels:
        self.frame_make_payment_title = Label(self.frame_make_payment, text='Make your Payment', font=FONT_TOP)
        ...
        # show these labels:
        self.l_bike_id_show_2 = Label(self.frame_make_payment, text='{}'.format(self.payment_info_data[1]), font=FONT)
        self.l_bike_type_show_2 = Label(self.frame_make_payment, text='{}'.format(self.payment_info_data[7]), font=FONT)
        self.l_start_time_show_2 = Label(self.frame_make_payment, text='{}'.format(self.payment_info_data[4]),
                                         font=FONT)
        self.l_end_time_show_2 = Label(self.frame_make_payment, text='{}'.format(self.payment_info_data[5]), font=FONT)
        self.l_riding_time_show_2 = Label(self.frame_make_payment, text='{}'.format(self.payment_info_data[6]),
                                          font=FONT)
        self.l_price_show_2 = Label(self.frame_make_payment, text='£{:.2f}'.format(self.price), font=FONT)
        ...
        S_W = self.master.winfo_screenwidth()
        S_H = self.master.winfo_screenheight()
        self.frame_make_payment_title.place(x=S_W * 450 / 1440, y=S_H * (2 / 15), width=500)
        ...
        self.l_bike_id_show_2.place(x=S_W * 400 / 1440, y=S_H * (4 / 15), )
        self.l_bike_type_show_2.place(x=S_W * 1000 / 1440, y=S_H * (4 / 15))
        self.l_start_time_show_2.place(x=S_W * 400 / 1440, y=S_H * (6 / 15))
        self.l_end_time_show_2.place(x=S_W * 1000 / 1440, y=S_H * (6 / 15))
        self.l_riding_time_show_2.place(x=S_W * 400 / 1440, y=S_H * (8 / 15))
        self.l_price_show_2.place(x=S_W * 1000 / 1440, y=S_H * (8 / 15))
        # place button:
        self.btn_comf_pay.place(x=S_W * 570 / 1440, y=S_H * 650 / 900, width=WIDTH, height=HEIGHT)
...
class Report:
    ...
    def show_deatil(self):
        ...
        Label(canvas, text='Report_time: {}'.format(record[-4]), font=('Arial', 14)).place(x=30, y=30)
        Label(canvas, text='Location：{}'.format(record[5]), font=('Arial', 14)).place(x=30, y=80)
        Label(canvas, text='Error_type：{}'.format(record[6]), font=('Arial', 14)).place(x=30, y=130)
        Label(canvas, text='Desc：{}'.format(record[-1]), font=('Arial', 14)).place(x=30, y=180)
        Label(canvas, text='Status：{}'.format(record[8]), font=('Arial', 14)).place(x=30, y=230)
        canvas.pack()
        top1.mainloop()
    ...
    def freport(self):
        ...
        S_W = self.master.winfo_screenwidth()
        S_H = self.master.winfo_screenheight()
        self.btn_new_report.place(x=S_W * 570 / 1440, y=S_H * 300 / 1440, width=WIDTH, height=HEIGHT)
        self.btn_pro.place(x=S_W * 570 / 1440, y=S_H * 500 / 1440, width=WIDTH, height=HEIGHT)
    ...
    def fcreate_new_rp(self):
        ...
        self.l_time_show.place(x=S_W * (11 / 25), y=S_H * (2 / 15))
        ...
        self.comb_bike_type.place(x=S_W * (11 / 25), y=S_H * (6 / 15), width=S_W * (200 / 1440),
                                  height=S_H * (30 / 900))
        self.l_desc.place(x=S_W * (6 / 25), y=S_H * (8 / 15))
        self.e_desc.place(x=S_W * (11 / 25), y=S_H * (8 / 15), width=S_W * (400 / 1440), height=S_H * (100 / 900))
        self.comb_err_type.place(x=S_W * (145 / 250), y=S_H * (6 / 15), width=S_W * (200 / 1440),
                                 height=S_H * (30 / 900))
    ...
    def fprocessing(self):
        ...
        S_W = self.master.winfo_screenwidth()
        S_H = self.master.winfo_screenheight()
        Label(self.frame_processing, text='Error Journals', font=FONT_TOP).place(x=S_W * 600 / 1440, y=S_H * 150 / 900)
...
class My:
    ...
    def edit_name(self):
        ...
        self.entry_username.place(x=S_W * (9.5 / 14), y=S_H * (15 / 80), width=150, height=HEIGHT)
    ...
    def GBP_others(self):
        ...
        self.entry_other_GBP.place(x=S_W * 550 / 1440, y=S_H * (23 / 90), width=150, height=HEIGHT)
    ...
    def confirm_pay(self):
        ...
        click = messagebox.showinfo("Payment Accomplished", "Successful Payment!")
    ...
    def check_old_password(self):
        ...
        if entry_password == my_password:
            ...
            click = messagebox.showerror("Error", "Password is not matched! Please try again")
            if click:
    ...
    def confirm_new_password(self):
        ...
        if new_entry == repeat_entry:
            ...
            messagebox.showinfo("Success Window", "Success!")
            self.show_frame_my()
        else:
            messagebox.showerror("Error", "Your entry passwords are not matched!")
    ...
    def ShowAddCard(self):
        ...
        self.l_card_num.place(x=150, y=100)
        self.l_cv_num.place(x=150, y=150, )
        self.l_expired_date.place(x=150, y=200)
        self.l_holder.place(x=150, y=250)
        self.l_hit.place(x=340, y=300)
        self.e_card_num.place(x=340, y=100)
        self.e_cv_num.place(x=340, y=150)
        self.e_expired_date.place(x=340, y=200)
    ...
    def insert_new_card(self):
        ...
        click = messagebox.showinfo("New Card inserted",
                                    "Successfully added a new card, please go back.")
        if click:
            self.add_card.destroy()
            self.show_frame_my()
    ...
    def fmy(self):
        ...
        self.l_user_name_show_1.place(x=S_W * (9.5 / 14), y=S_H * (4 / 20))
        ...
        # place buttons:
        self.btn_edit_name.place(x=S_W * (11 / 14), y=S_H * (39 / 200), width=100, height=40)
        self.btn_confirm_newName.place(x=S_W * (12 / 14), y=S_H * (39 / 200), width=100, height=40)
        self.btn_edit_email.place(x=S_W * (8 / 14), y=S_H * (59 / 200), width=100, height=40)
        self.btn_confirm_newEmail.place(x=S_W * (9 / 14), y=S_H * (59 / 200), width=100, height=40)
        self.btn_top_up.place(x=S_W * (3 / 14), y=S_H * (20 / 40), width=WIDTH, height=50)
        self.btn_transaction_history.place(x=S_W * (3 / 14), y=S_H * (24 / 40), width=WIDTH, height=50)
        self.btn_rent_history.place(x=S_W * (3 / 14), y=S_H * (28 / 40), width=WIDTH, height=50)
        self.btn_report_history.place(x=S_W * (8 / 14), y=S_H * (28 / 40), width=WIDTH, height=50)
        self.btn_change_password.place(x=S_W * (8 / 14), y=S_H * (20 / 40), width=WIDTH, height=50)
    ...
    def ftop_up(self):
        ...
        self.l_top_up_amount_show.place(x=S_W * 600 / 1440, y=S_H * (24 / 90))
        ...
        self.btn_confirm_otherGBP.place(x=S_W * (8 / 14), y=S_H * (23 / 90), width=WIDTH, height=50)
    ...
    def fpayment_selection(self):
        ...
        # create labels:
        self.l_pay_by_your_card = Label(self.frame_payment_selection, text="Please pay by your card", font=FONT_TOP)
        self.l_card = Label(self.frame_payment_selection, text="Card:", font=FONT_TOP)
        ...
        self.l_pay_by_your_card.place(x=S_W * (40 / 140), y=S_H * (30 / 90))
        self.l_card.place(x=S_W * (40 / 140), y=S_H * 650 / 1440)
        self.optionmenu_card.place(x=S_W * (50 / 140), y=S_H * 650 / 1440, width=S_W * (300 / 1440),
                                   height=S_H * (50 / 900))
        self.btn_go_back.place(x=S_W * (20 / 140), y=S_H * (60 / 90), width=WIDTH, height=S_H * (50 / 900))
        self.btn_pay.place(x=S_W * 900 / 1440, y=S_H * (60 / 90), width=WIDTH, height=S_H * (50 / 900))
        self.btn_add_newcard.place(x=S_W * 900 / 1440, y=S_H * (413 / 900), width=WIDTH, height=S_H * (50 / 900))
    ...
    def ftransaction_history(self):
        ...
        self.optionmenu_transaction_type = ttk.Combobox(self.frame_transaction_history, values=types,
                                                        textvariable=self.selected_transaction_type, font=FONT)
    ...
    def frent_history(self):
        ...
        self.l_rent_date = Label(self.frame_rent_history, text="Start Location: ", font=FONT)
        ...
        self.optionmenu_bike_type = ttk.Combobox(self.frame_rent_history, values=types,
                                                 textvariable=self.selected_rentbike_type, font=FONT)
        select_parking_location_1 = CusOpenDatabase()
        parking_location = select_parking_location_1.select_parking_location()
        ...
        self.optionmenu_rent_location = ttk.Combobox(self.frame_rent_history, values=parking_location,
                                                     textvariable=self.select_rent_location, font=FONT)
        # create button:
        ...
        self.l_rent_history_type.place(x=S_W * 200 / 1440, y=S_H * (15 / 90))
        self.l_rent_date.place(x=S_W * 750 / 1440, y=S_H * (15 / 90))
        self.optionmenu_bike_type.place(x=S_W * 350 / 1440, y=S_H * (14 / 90), width=300, height=50)
        self.optionmenu_rent_location.place(x=S_W * 950 / 1440, y=S_H * (14 / 90), width=300, height=50)
        self.btn_go_back.place(x=S_W * (2 / 14), y=S_H * (60 / 90), width=150, height=50)
    ...
    def freport_history(self):
        ...
        self.optionmenu_location = ttk.Combobox(self.frame_report_history, values=parking_location,
                                                textvariable=self.report_history_location, font=FONT)
        ...
        self.optionmenu_report_type = ttk.Combobox(self.frame_report_history, values=error_types,
                                                   textvariable=self.report_err_type, font=FONT)
        ...
        self.l_report_type.place(x=S_W * (7 / 14), y=S_H * (15 / 90))
        self.optionmenu_location.place(x=S_W * 350 / 1440, y=S_H * (143 / 900), width=300, height=50)
        self.optionmenu_report_type.place(x=S_W * 900 / 1440, y=S_H * (143 / 900), width=300, height=50)
        self.btn_go_back.place(x=S_W * (2 / 14), y=S_H * (60 / 90), width=150, height=50)
        self.report_go_query.place(x=S_W * 1050 / 1440, y=S_H * (60 / 90), width=150, height=50)
    ...
    def fchange_password(self):
        ...
        self.l_change_password_title = Label(self.frame_change_password, text="Please enter your current password",
                                             font=FONT_TOP)
        # create Entry:
        self.e_old_password = Entry(self.frame_change_password, text="", show="*", font=FONT_TOP)
        # create button:
        ...
        self.l_change_password_title.place(x=S_W * 500 / 1440, y=S_H * (30 / 90))
        self.e_old_password.place(x=S_W * 530 / 1440, y=S_H * (4 / 9), width=400, height=50)
    ...
    def fnew_password(self):
        ...
        # create labels:
        self.l_enter_new_password = Label(self.frame_new_password, text="Enter new Password:", font=FONT_TOP)
        self.l_repeat_password = Label(self.frame_new_password, text="Repeat Password:", font=FONT_TOP)
        # create a Entry:
        self.e_new_password = Entry(self.frame_new_password, text="", show="*", font=FONT)
        self.e_repeat_password = Entry(self.frame_new_password, text="", show="*", font=FONT)
...
class CustomerUI:
    ...
    def __init__(self) -> None:
        ...
        # place buttons:
        self.btn_rent.place(x=0, y=0, width=S_W / 3., height=HEIGHT)
        self.btn_report.place(x=S_W / 3, y=0, width=S_W / 3., height=HEIGHT)
        self.btn_my.place(x=S_W / 3 * 2, y=0, width=S_W / 3., height=HEIGHT)
    ...
    def show_frame_rent(self):
        self.forget_all()
        self.reset_btn_bg()
        self.btn_rent.configure(bg='lightpink')
        self.frame_rent.pack()

    def show_frame_report(self):
        self.forget_all()
        self.reset_btn_bg()
        self.btn_report.configure(bg='lightpink')
        self.frame_report.pack()
