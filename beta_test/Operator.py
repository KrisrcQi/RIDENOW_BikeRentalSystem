    ...
    def getChargeData(self):
        ...
        if self.selected_location.get() != "Choose Location" and \
            ...
            chargeData = DB.get_chargeData(self.selected_location.get(),
                                           self.selected_type1.get())
        elif self.selected_location.get() == "Choose Location" and \
                self.selected_type1.get() != "Choose Type":
            chargeData = DB.get_chargeData_t(self.selected_type1.get())
        elif self.selected_location.get() != "Choose Location" and \
                self.selected_type1.get() == "Choose Type":
            chargeData = DB.get_chargeData_l(self.selected_location.get())
        else:
...
class ChargeDetails:
    ...
    def confim_charge(self):
        ...
        if result:  # if the user clicked the button
...
class ChargeHistoryDetails:
    """showing the charge history frame"""
...
class RepairHistoryDetails:
    """showing repair history details"""
    ...
    def init_ui(self):
        ...
        self.m_description = Message(self.master, text=text, font=font, width=380)
...
class Move:
    ...
    #     self.l_date = Label(self.frame_move_confirm, text='{}'.format(datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
    #                         font=FONT)
    #     self.l_date.place(x=S_W/1440*(300+200), y=S_H/900*450)
    #
    #     # click each row and go into the detail of each moving window
...
class MoveDetails:
    """showing the moving details"""

    def confim_move(self):
        result = messagebox.askyesno("Confirm", "Confirm Move？")
        if result:
            moving = OperatorDatabase()
            toLocation = self.mlocation.get()
            bike_id = self.record.BID
            fromLocation = self.record.Location
            bike_type = self.record.BType
            moving.movingbike(bike_id, bike_type, fromLocation, toLocation)
            messagebox.showinfo("success window", "Move Successful !")
        self.master.destroy()
    ...
    def init_ui(self):
        ...
        self.c_moveto = ttk.Combobox(self.master, font=font,
                                     values=self.get_location(), textvariable=self.mlocation)
        self.c_moveto.place(x=200, y=220, width=300)

        self.l_time = Label(self.master, text='time:', font=font)
        self.l_time.place(x=80, y=270)
        self.e_time = Entry(self.master, font=font)
        self.e_time.insert(0, datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        self.e_time.configure(state='readonly')
        self.e_time.place(x=200, y=270, width=300)
...
class MoveHistoryDetails:
    """showing the moving history details"""
